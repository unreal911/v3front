...
export class PrincipalComponent implements OnInit {
  ...
  obtenerfechaActual() {
    ...
    this.initDia = `${startDate.getFullYear()}-${startDate.getMonth() + 1}-${startDate.getDate()}`
  ...
  getSemana() {
    this.busquedaService.getSemana({ fechaInicio: this.initDia, fechaFin: this.finDia }).subscribe({
      next: (r: any) => {
        ...
        this.formatoSubmit.forEach(dia => {
          ...
          let ventaEncontrada = this.resultados.find((venta: any) => venta.fecha == dia);
        ...
        for (let i = 0; i < this.infoTransformada.length; i++) {
          if (this.infoTransformada[i].ventas.VentaGenerada) {
            this.ventagenerada.push(this.infoTransformada[i].ventas.VentaGenerada)
          } else {
