...
export class PedidoComponent implements OnInit {
  ...
  agregarFiltros(tipoFiltro: string, valor: any) {
    if (tipoFiltro == 'tipoventa') {
      let encontrarFiltros = this.filtroMongo.tipoventa!.$in.find(
        (r) => r == valor
      );
      if (!encontrarFiltros) {
        ...
        this.filtroMongo.tipoventa!.$in =
          this.filtroMongo.tipoventa!.$in.filter((tipo) => tipo != valor);
      }
    } else if (tipoFiltro == 'pagado') {
      let encontrarFiltros = this.filtroMongo.pagado!.$in.find(
        (r) => r == valor
      );
      if (!encontrarFiltros) {
  ...
  clickSave(x: number, campo: string, pedido: any) {
    if (campo == 'nombre') {
      ...
      this.pedidosServices
        .actualizarPedido(pedido.uid, { nombre: pedido.nombre })
        .subscribe({
          next: (r) => {
            this.toastr.success(
              'Cambio realizado!!!',
      ...
      this.pedidosServices
        .actualizarPedido(pedido.uid, { correo: pedido.correo })
        .subscribe({
          next: (r) => {
            this.toastr.success(
              'Cambio realizado!!!',
      ...
      this.pedidosServices
        .actualizarPedido(pedido.uid, { telefono: pedido.telefono })
        .subscribe({
          next: (r) => {
            this.toastr.success(
              'Cambio realizado!!!',
      ...
      if (result.isConfirmed) {
        this.pedidosServices.eliminar(pedido.uid).subscribe({
          next: (r: any) => {
            console.log(r);
            this.listarPedidos();
            Swal.fire('Elimiado!', 'El suaurio fue eliminado.', 'success');
          },
          error: (e: any) => {
            console.log(e);
            Swal.fire('Info!', e.error.msg, 'error');
          },
        });
      }
    });
  ...
  cambiarEstado(pedido: any) {
    this.pedidosServices
      .cambiarEstado(pedido.uid, { estado: pedido.estado })
      .subscribe({
        next: (r) => {
          console.log(r);
  ...
  cambiarPagado(pedido: any) {
    this.pedidosServices
      .cambiarPagado(pedido.uid, { pagado: pedido.pagado })
      .subscribe({
        next: (r) => {
          console.log(r);
          this.toastr.success(
            'Cambio realizado!!!',
            'Se cambio el estado Pagado'
          );
        },
        error: (e) => {
          console.log(e);
