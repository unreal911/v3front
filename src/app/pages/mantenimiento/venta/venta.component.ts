...
export class VentaComponent implements OnInit {
  ...
  GenerarPedido() {
    ...
    for (let i = 0; i < arrayP.length; i++) {
      if (arrayP[i].cantidad < 1) {
        Swal.fire(
          'Formulario Invalido',
          'El cantidad no puede ser menor a 1',
          'error'
        );
        return;
      }
    ...
    if (this.formPedido.invalid) {
      Swal.fire(
        'Formulario Invalido',
        'El formulario aun falta completar',
        'error'
      );
      return;
    }
    this.pedidoService.crearVentaPedido(this.formPedido.value).subscribe({
      next: (n: any) => {
        ...
        for (let i = 0; i < this.arrayProductos.value.length; i++) {
          ...
          this.pedidoService
            .crearDetalleVentaPedidoP(this.arrayProductos.value[i])
            .subscribe({
              next: (r) => {
                console.log(r);
              },
  ...
  getColorClass(productControl: AbstractControl) {
    ...
    if (typeof valor === 'number') {
      return {
        ...
        'is-invalid':
          (productControl.invalid && !productControl.pristine) ||
          productControl.value < 1,
      };
    } else {
      return {
        'is-valid': productControl.valid && !productControl.pristine,
  ...
  validarCampo(nombre: string) {
    if (
      this.formPedido.get(nombre)?.pristine == false &&
      this.formPedido.get(nombre)?.valid == true
    ) {
      return true;
    } else {
      return false;
  ...
  campoIncorrecto(campo: string) {
    if (
      this.formPedido.get(campo)?.pristine == false &&
      this.formPedido.get(campo)?.invalid == true
    ) {
      return true;
    } else {
      return false;
